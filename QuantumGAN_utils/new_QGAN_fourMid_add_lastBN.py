...
class QuantumGenerator(nn.Module):
    def __init__(self, KernelAncilla, layer_num, 
                 ...
                 stage_5_dim, stage_5_input_size, stage_5_kernel_size, stage_5_stride, stage_5_padding,
                 final_dim, final_size):  
        ...
        self.BatchNorm2d_stage_5_dim = nn.BatchNorm2d(stage_5_dim)
        ...
        self.stage_5_dim = stage_5_dim
        self.stage_5_input_size = stage_5_input_size
        self.stage_5_kernel_size = stage_5_kernel_size
        self.stage_5_stride = stage_5_stride
        self.stage_5_padding = stage_5_padding   
        ...
        self.kernel_qubit_5 = KernelAncilla + int(np.ceil(math.log2(stage_5_kernel_size[0]*stage_5_kernel_size[1]))) 
        ...
        # -------------------------------- stage 5: 8*13*13 --> 1*16*16 --------------------------------

        self.stage_5_qcnn_node = create_qcnn_node(KernelAncilla=KernelAncilla,
                                                  output_size=final_size, 
                                                  kernel_size=stage_5_kernel_size,
                                                  layer_num=layer_num)
        self.tmp_layer_5 = []
        for ii in range(stage_5_dim):
            self.tmp_layer_5.append(qml.qnn.TorchLayer(self.stage_5_qcnn_node, {"weights": (layer_num, self.kernel_qubit_5)}))
                
        self.qgen_stage_5 = nn.Sequential(*self.tmp_layer_5)
    ...
    def forward(self, z):
        ...
        # qcnn_stage_4_output: (batch_size, stage_5_dim, stage_5_input_size[0], stage_5_input_size[1])
        qcnn_stage_4_output = qcnn(KernelAncilla=self.KernelAncilla, input_feature=qcnn_stage_4_input, 
                                   output_dim=self.stage_5_dim, output_size=self.stage_5_input_size, 
        ...
        # qcnn_stage_5_input: (batch_size, stage_5_dim, stage_5_input_size[0], stage_5_input_size[1])
        qcnn_stage_5_input = self.ReLU(self.BatchNorm2d_stage_5_dim(qcnn_stage_4_output))
        ...
        # qcnn_stage_5_output: (batch_size, final_dim, final_size[0], final_size[1])
        qcnn_stage_5_output = qcnn(KernelAncilla=self.KernelAncilla, input_feature=qcnn_stage_5_input, 
                                   output_dim=self.final_dim, output_size=self.final_size, 
                                   kernel_size=self.stage_5_kernel_size, stride=self.stage_5_stride, 
                                   padding_index=self.stage_5_padding, qcnn_layer=self.qgen_stage_5)


        qcnn_stage_5_output_add_lastBN = self.BatchNorm2d_final_dim(qcnn_stage_5_output)


        # final_output: (batch_size, final_dim, final_size[0], final_size[1])
        final_output = self.Tanh(qcnn_stage_5_output_add_lastBN)


        return final_output, qcnn_stage_5_output, qcnn_stage_5_output_add_lastBN
